...
def _classify_connection_error(exc: psycopg.Error) -> str:
    """
    Return a safe error classification without exposing credentials,
    connection strings, usernames, passwords, or SQL contents.
    """
    ...
    if (
        "no pg_hba.conf entry" in message
        or "pg_hba" in message
    ):
...
def connect(
    ...
    *,
    ...
    kwargs = {
        ...
        "connect_timeout": 10,
        "application_name": "suam_pipeline",
        "options": (
    ...
    sslmode = value("POSTGRES_SSLMODE")
    ...
    try:
        ...
        classification = _classify_connection_error(
            exc
        )
...
def inspect_table(
    ...
    if table not in {
        ...
        raise ValueError(
            "Table is outside Suam's write scope"
        )
